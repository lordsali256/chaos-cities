...
City listings and event feeds require a city key. The standalone feed contains only events involving that city; personal resources and full traits remain in the private city response.
...
**Citizens and appearance:** Every population point has a named resident with one of 36 races. A local model replaces fallback names in small batches when available. Each race has a preferred enemy and deals double damage in a matching battle duel. Equipped special citizens retain their daily stat bonus and add tier-based battle power and matching trait bonuses; they gain an extra bonus when a fighter from their ally race is present. Each special citizen also has a stable combat role: Guard improves defense, Rally strengthens other equipped citizens, and Focus boosts sampled traits in its chosen domain. The battle record shows what each citizen contributed. Battle odds still stay within 35–65%. The Help tab explains the rules. The compact five-stat strip stays visible on every tab. A city can be renamed, and its dark-friendly scenery follows one of 72 gradual landscape and mood paths over the 90-day reality ramp.
...
2. Copy `.env.example` to `.env`. Set `GAME_DOMAIN` to a domain name whose DNS record points to your host. Set a private, randomly generated `INVITE_CODE` of at least 20 characters and give that code only to friends. The public Compose configuration refuses to start without both values and sets transfer QRs to `https://GAME_DOMAIN`. Do not put the code in a public repository. Public registration is limited to five new cities per public IP address in 24 hours; the proxy overwrites the client IP header before forwarding. Friends sharing one internet connection share that allowance. `DAY_SECONDS=86400` means a real economy day and `TOKEN_SECONDS=3600` means one token hour. `AMBIENT_SECONDS` controls City AI incident cadence.
...
- Invite code is shared and reusable. It keeps strangers out of small friend games but is not a full account system. The public sign-up cap slows extra-city creation but cannot guarantee one human per city.
