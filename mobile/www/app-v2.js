...
function render() {
  ...
  app.innerHTML = `<div class="shell">
    ...
    <div class="surface">${rivals.length ? rivals.map(item => `<div class="row"><div><div class="city-name">${esc(item.name)}</div><div class="muted">Mayor ${esc(item.owner_name)} · ${item.population} citizens · ${state.battleRecords[item.id]?.wins || 0} W / ${state.battleRecords[item.id]?.losses || 0} L · Your attacks ${state.pairAttacks[item.id] || 0}/${state.pairBattleLimit} · Incoming ${item.incoming_attacks_24h || 0}/${item.incoming_attack_limit || 6}${Date.now()/1000 < item.hero_loss_protection_until ? ` · Heroes safe for ${clock(item.hero_loss_protection_until)}` : ''}</div></div><span class="chip">${Date.now()/1000 < item.battle_shield_until ? `Protected ${clock(item.battle_shield_until)}` : !attackable(item) ? 'Daily defense limit' : (state.pairAttacks[item.id] || 0) >= state.pairBattleLimit ? 'Your daily limit' : `🌀 ${item.weirdness}`}</span></div>`).join('') : '<p class="empty">No rivals yet. Invite a friend with your server address.</p>'}</div>
    ...
    <div class="surface battle-panel"><div><strong>⚔️ Challenge another city</strong><p class="muted">Costs ${city.battle_cost} Wealth. Challengers rest 4 hours; new cities and recent defenders get 1 hour of protection. You can challenge the same city twice per 24 hours. A city can receive at most ${city.incoming_attack_limit || 6} hostile events and lose at most one Hero to battles in 24 hours.${Date.now()/1000 < city.hero_loss_protection_until ? ` Your remaining Heroes are safe for ${clock(city.hero_loss_protection_until)}.` : ''}</p></div><button class="cast" onclick="startBattle()" ${clock(city.next_battle_at) !== 'ready' || city.wealth < city.battle_cost || !rivals.some(item => attackable(item) && (state.pairAttacks[item.id] || 0) < state.pairBattleLimit) ? 'disabled' : ''}>Choose a rival →</button></div>
    ` : ''}
    ${state.page === 'trades' ? `
